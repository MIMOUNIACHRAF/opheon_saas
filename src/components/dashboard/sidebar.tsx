...
export function DashboardSidebar({ tenant }: SidebarProps) {
  ...
  const links = [
    ...
    { href: `${base}/commandes`,      label: 'Commandes',      icon: ShoppingBag },
    { href: `${base}/reservations`,   label: 'Réservations',   icon: CalendarDays },
    { href: `${base}/clients`,        label: 'Clients',        icon: Users },
    { href: `${base}/analytics`,      label: 'Statistiques',   icon: BarChart3 },
  ...
  return (
    <aside
      className={cn(
        'flex flex-col h-screen bg-charcoal border-r border-white/10 transition-all duration-300',
        collapsed ? 'w-16' : 'w-64'
      )}
    >
      {/* Logo */}
      ...
      {/* Bottom */}
      <div className={cn('p-2 border-t border-white/10 space-y-1', collapsed && 'items-center')}>
        <Link
          href={`/${tenant.slug}`}
          className={cn(
